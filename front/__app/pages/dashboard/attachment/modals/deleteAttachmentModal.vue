...
function submitDeleteForm(e: Event) {
  ...
  toast.promise(
    axios.delete(apiRoutes.deleteAttachment(props.attachment.id)),
    {
      loading: `Suppression en cours de la pièce ...`,
      success: (res: any) => {
        ...
        return 'Pièce supprimée avec succès!'
      },
    ...
    <DialogContent class="sm:max-w-sm text-center bg-light">
      <DialogHeader>
        ...
        <DialogDescription>
          ...
          Cette commentaire <span class="badge bg-light border rounded text-dark">{{ demand?.code }}</span> sera
