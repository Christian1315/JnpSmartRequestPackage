...
// ── Le role à modifier, passé en prop depuis le tableau ────
...
function fillFormFromUser() {
  ...
  data.name = props.role.name || ''
  data.description = props.role.description || ''
}

// Si le role sélectionné change pendant que le modal est déjà ouvert
watch(() => props.role, () => {
...
async function submitUpdateForm(e: Event) {
  ...
  try {
    await toast.promise(
      axios.put(apiRoutes.updateRole(props.role.id), {
        name: data.name,
        description: data.description,
      }),
      {
        loading: 'Modification du rôle',
        success: (res: any) => {
