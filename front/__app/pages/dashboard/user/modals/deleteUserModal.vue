...
const props = defineProps<{
  user:{
    id: number
    roleId: number | null
    fullname: { id: number, code: string } | null
    email: { id: number, code: string } | null
    phone: { id: number, code: string } | null
...
function submitDeleteForm(e: Event) {
  ...
  toast.promise(
    axios.delete(apiRoutes.deleteUser(props.user.id)),
    {
      loading: `Suppression en cours du compte ...`,
      success: (res: any) => {
    ...
    <DialogContent class="sm:max-w-sm text-center bg-light">
      <DialogHeader>
        ...
        <DialogDescription>
          ...
          Ce compte <span class="badge bg-light border rounded text-dark">{{ props?.user?.fullname }}</span> sera supprimé définitivement.
        </DialogDescription>
      </DialogHeader>
