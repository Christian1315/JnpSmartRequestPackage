...
const props = defineProps<{
  ...
  comment: { id: number, code: string } | null
  createdBy: {
    id: number
    fullname: string
  } | null
...
function submitDeleteForm(e: Event) {
  ...
  toast.promise(
    axios.delete(apiRoutes.deleteComment(props.comment.id)),
    {
      loading: `Suppression en cours du commantaire ...`,
      success: (res: any) => {
    ...
    <DialogContent class="sm:max-w-sm text-center bg-light">
      <DialogHeader>
        ...
        <DialogDescription>
          ...
          Cette commentaire <span class="badge bg-light border rounded text-dark">{{ demand?.code }}</span> sera supprimée définitivement.
        </DialogDescription>
      </DialogHeader>
