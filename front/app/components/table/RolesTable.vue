...
import AddRoleModal from '~/pages/dashboard/role/modals/addRoleModal.vue'
import UpdateRoleModal from '~/pages/dashboard/role/modals/updateRoleModal.vue'
import DeleteRoleModal from '~/pages/dashboard/role/modals/deleteRoleModal.vue'
...
const fatchRoles = async () => {
  ...
  try {
    ...
    const response = await axios.get(apiRoutes.allRole)
    ...
    console.error('Roles chargement error:', apiMessage)
...
const columns: ColumnDef<Role>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const items = role.id != 1 ? [
        h(DropdownMenuLabel, null, () => 'Actions'),
        h(DropdownMenuSeparator),
        h(DropdownMenuItem, {
          class: 'text-warning',
          style: { cursor: 'pointer' },
          onSelect: (e: Event) => { e.preventDefault(); onEdit(role) },
        }, () => [h(PencilLine), ' Modifier']),
        h(DropdownMenuItem, {
          class: 'text-danger',
          style: { cursor: 'pointer' },
          onSelect: (e: Event) => { e.preventDefault(); onDelete(role) },
        }, () => [h(Eraser), ' Supprimer']),
      ] : []
...
<template>
  ...
  <div v-else-if="error" class="text-center py-8 text-destructive">
    {{ error }}
    <Button variant="outline" size="sm" class="ml-2" @click="fatchRoles()">
      Réessayer
    </Button>
  </div>
  ...
  <div v-else class="space-y-4">
    ...
    <!-- les modals -->
    <AddRoleModal v-model:open="isModalOpen" @created="fatchRoles" />
    <UpdateRoleModal v-model:open="isUpdateModalOpen" :role="currentRole" @updated="fatchRoles" />
    <DeleteRoleModal v-model:open="isDeleteModalOpen" :role="currentRole" @deleted="fatchRoles" />
  </div>
</template>