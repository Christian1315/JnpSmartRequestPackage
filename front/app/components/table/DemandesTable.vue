...
//   getSortedRowModel, getVisibilityRowModel, type ColumnDef, type VisibilityState, FlexRender,
...
const columns: ColumnDef<Demande>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      // à l'état brouillon
      if (!demand.statut || demand.statut.id == 1 || demand.statut.id == 5) {
        // modifer
        items.push(
          h(DropdownMenuItem, {
            class: 'text-warning',
          ...
          h(DropdownMenuItem, {
            class: 'text-danger',
            style: { cursor: 'pointer' },
            onSelect: (e: Event) => { e.preventDefault(); onDelete(demand) },
          }, () => [h(Eraser), ' Supprimer']),
        )
      }
